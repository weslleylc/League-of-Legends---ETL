...
def main():
    ...
    spark, log, config = start_spark(
       ...
       jar_packages=['org.apache.hadoop:hadoop-aws:2.7.3'],
       files=['configs/etl_config.json'])
    AWS_ACCESS_KEY = "AKIAJONAFAPDPIT6NX5A"
    AWS_SECRET_KEY = "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"
    ...
    stats_df = spark.read.csv("s3://dsp-ch6-weslley/csv/game_skater_stats.csv", header=True, inferSchema = True)
    print(stats_df.show())
...
def extract_data(spark):
    ...
    # read match data
    match_data = spark.read.csv("/home/weslley/Documents/Projects/lol/pyspark-example-project/data/match_data_version1.csv",
		             header='true',
		             schema=match_schema)
    match_data = match_data.sample(False, 0.1, seed=0)

	# https://www.kaggle.com/tk0802kim/kerneld01a1ec7ad
    itens = spark.read.csv("/home/weslley/Documents/Projects/lol/pyspark-example-project//data/riot_item.csv",
		             header='true',
		             schema=itens_schema)

    champions = spark.read.csv("/home/weslley/Documents/Projects/lol/pyspark-example-project//data/riot_champion.csv",
		             header='true',
...
def transform_data(spark, match_data, itens, champions, steps_per_floor_):
...
def load_data(spark, players, champions, build_first_item, build):
    """Collect data locally and write to CSV.

    :param df: DataFrame to print.
    :return: None
    """
    players.write.parquet("/home/weslley/Documents/Projects/lol/pyspark-example-project//players.parquet")
    champions.write.parquet("/home/weslley/Documents/Projects/lol/pyspark-example-project//champions.parquet")
    build_first_item.write.parquet("/home/weslley/Documents/Projects/lol/pyspark-example-project//build_first_item.parquet")
    build.write.parquet("/home/weslley/Documents/Projects/lol/pyspark-example-project//build.parquet")
    return None
